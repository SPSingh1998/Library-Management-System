...
public class STUDENT_CHANGE_PASSWORD extends javax.swing.JFrame {
    String id;
    public STUDENT_CHANGE_PASSWORD(String c) {
        initComponents();
        id=c;
    }
    ...
    private void jButton1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton1ActionPerformed
        try{
            String qry="select stupwd from tbstudent where stuid='"+id+"'";
            ResultSet rs=new DB().execute(qry);
            rs.next();
            if(rs.getString("stupwd").equals(jPasswordField1.getText()))
            {
                if(jPasswordField2.getText().equals(jPasswordField3.getText()))
                {
                    qry="update tbstudent set stupwd='"+jPasswordField2.getText()+"'";
                    new DB().update(qry);
